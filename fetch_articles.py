#!/usr/bin/env python3
import json
import os
import re
import time
import urllib.request
import xml.etree.ElementTree as ET
from datetime import datetime, timezone
from email.utils import parsedate_to_datetime

SOURCES = [
    ("DATAVERSITY", "https://www.dataversity.net/category/blogs/feed/", "news"),
    ("Nicola Askham", "https://www.nicolaaskham.com/blog?format=rss", "opinions"),
    ("TDAN.com", "https://tdan.com/feed/", "news"),
    ("LightsOnData", "https://lightsondata.com/feed/", "tools"),
    ("Henrik Liliendahl", "https://liliendahl.com/feed/", "opinions"),
    ("Privacy International", "https://privacyinternational.org/rss.xml", "regulation"),
    ("Merudata Blog", "https://merudata.com/blog-feed.xml", "news"),
    ("InformationBytes", "https://informationbytes.com/feed/", "regulation"),
    ("Perficient DG", "https://blogs.perficient.com/tag/data-governance/feed/", "tools"),
    ("SmartData Collective", "https://smartdatacollective.com/feed/", "news"),
    ("IAPP News", "https://iapp.org/news/rss/", "regulation"),
    ("KDnuggets", "https://kdnuggets.com/feed", "news"),
    # Substack publications
    ("The Data Ecosystem (Substack)", "https://thedataecosystem.substack.com/feed", "opinions"),
    ("DG Playbook - Charlotte Ledoux (Substack)", "https://thedatagovernanceplaybook.substack.com/feed", "opinions"),
    ("Metadata Weekly - Atlan (Substack)", "https://metadataweekly.substack.com/feed", "news"),
    ("PrivacyX - Steve Wood (Substack)", "https://privacyx.substack.com/feed", "regulation"),
    ("DG Privacy & Security (Substack)", "https://dbadminnews.substack.com/feed", "regulation"),
]

MAX_ARTICLES = 8
TOTAL_CAP = 60
ANTHROPIC_API_KEY = os.environ.get("ANTHROPIC_API_KEY", "")


def fetch_rss(url, timeout=10):
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "DataGovernanceDigest/1.0"})
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            return resp.read()
    except Exception as e:
        print("  [WARN] Could not fetch " + url + ": " + str(e))
        return None


def parse_rss(xml_bytes, source_name, default_section):
    articles = []
    try:
        root = ET.fromstring(xml_bytes)
        ns = {"atom": "http://www.w3.org/2005/Atom"}
        channel = root.find("channel")
        if channel is None:
            entries = root.findall("atom:entry", ns) or root.findall("entry")
            for entry in entries[:MAX_ARTICLES]:
                title = (entry.findtext("atom:title", namespaces=ns) or entry.findtext("title") or "").strip()
                url = ""
                link = entry.find("atom:link", ns) or entry.find("link")
                if link is not None:
                    url = link.get("href") or link.text or ""
                summary = (entry.findtext("atom:summary", namespaces=ns) or entry.findtext("summary") or "").strip()
                date_str = (entry.findtext("atom:updated", namespaces=ns) or entry.findtext("updated") or "").strip()
                articles.append({
                    "title": strip_html(title),
                    "url": url.strip(),
                    "raw_summary": strip_html(summary)[:600],
                    "date": format_date(date_str),
                    "source": source_name,
                    "section": default_section,
                })
            return articles
        for item in channel.findall("item")[:MAX_ARTICLES]:
            title = strip_html(item.findtext("title", "")).strip()
            url = (item.findtext("link") or "").strip()
            summary = strip_html(item.findtext("description", "") or "").strip()[:600]
            date_str = item.findtext("pubDate", "").strip()
            articles.append({
                "title": title,
                "url": url,
                "raw_summary": summary,
                "date": format_date(date_str),
                "source": source_name,
                "section": default_section,
            })
    except ET.ParseError as e:
        print("  [WARN] XML parse error for " + source_name + ": " + str(e))
    return articles


def strip_html(text):
    text = re.sub(r'<[^>]+>', ' ', text or '')
    text = re.sub(r'&nbsp;', ' ', text)
    text = re.sub(r'&amp;', '&', text)
    text = re.sub(r'&lt;', '<', text)
    text = re.sub(r'&gt;', '>', text)
    text = re.sub(r'&quot;', '"', text)
    text = re.sub(r'&#\d+;', '', text)
    text = re.sub(r'\s+', ' ', text)
    return text.strip()


def format_date(date_str):
    if not date_str:
        return datetime.now(timezone.utc).strftime("%d %b %Y")
    try:
        dt = parsedate_to_datetime(date_str)
        return dt.strftime("%d %b %Y")
    except Exception:
        pass
    return date_str[:16]


def enrich_with_claude(articles):
    if not ANTHROPIC_API_KEY:
        print("[INFO] No API key - using raw summaries")
        for a in articles:
            a["summary"] = a["raw_summary"] or "Read the full article for details."
            a["tags"] = []
        return articles

    enriched = []
    batch_size = 6
    for i in range(0, len(articles), batch_size):
        batch = articles[i:i+batch_size]
        batch_text = "\n\n".join([
            "ARTICLE " + str(j+1) + ":\nTitle: " + a["title"] + "\nSource: " + a["source"] + "\nSection: " + a["section"] + "\nSummary: " + a["raw_summary"]
            for j, a in enumerate(batch)
        ])
        prompt = "You are an editor for a Data Governance Digest.\nFor each article return a JSON array with:\n- summary: 1-2 sentence summary max 180 chars\n- tags: 2-3 short tags\n- section: one of news, opinions, tools, regulation\n\nReturn ONLY valid JSON array, no markdown.\n\n" + batch_text

        try:
            payload = json.dumps({
                "model": "claude-sonnet-4-5-20250929",
                "max_tokens": 2000,
                "messages": [{"role": "user", "content": prompt}]
            }).encode()
            req = urllib.request.Request(
                "https://api.anthropic.com/v1/messages",
                data=payload,
                headers={
                    "Content-Type": "application/json",
                    "x-api-key": ANTHROPIC_API_KEY,
                    "anthropic-version": "2023-06-01",
                }
            )
            with urllib.request.urlopen(req, timeout=30) as resp:
                result = json.loads(resp.read())
                text = result["content"][0]["text"].strip()
                text = re.sub(r'^```(?:json)?\s*', '', text, flags=re.MULTILINE)
                text = re.sub(r'\s*```\s*$', '', text, flags=re.MULTILINE)
                parsed = json.loads(text.strip())
                enrichments = parsed if isinstance(parsed, list) else parsed.get("articles", [])
                for j, a in enumerate(batch):
                    if j < len(enrichments):
                        a["summary"] = enrichments[j].get("summary", a["raw_summary"])[:220]
                        a["tags"] = enrichments[j].get("tags", [])[:3]
                        a["section"] = enrichments[j].get("section", a["section"])
                    else:
                        a["summary"] = a["raw_summary"] or ""
                        a["tags"] = []
                enriched.extend(batch)
        except Exception as e:
            print("  [WARN] Enrichment failed: " + str(e))
            for a in batch:
                a["summary"] = a["raw_summary"] or ""
                a["tags"] = []
            enriched.extend(batch)
        time.sleep(0.5)
    return enriched


def main():
    print("Starting Data Governance Digest fetch...")
    all_articles = []

    for (name, url, section) in SOURCES:
        print("  Fetching: " + name)
        xml = fetch_rss(url)
        if xml:
            arts = parse_rss(xml, name, section)
            print("    -> " + str(len(arts)) + " articles")
            all_articles.extend(arts)
        time.sleep(0.3)

    seen = set()
    deduped = []
    for a in all_articles:
        if a["url"] and a["url"] not in seen:
            seen.add(a["url"])
            deduped.append(a)

    deduped = deduped[:TOTAL_CAP]
    print("  Total unique articles: " + str(len(deduped)))

    print("  Enriching with Claude AI...")
    enriched = enrich_with_claude(deduped)

    for a in enriched:
        a.pop("raw_summary", None)

    output = {
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "article_count": len(enriched),
        "articles": enriched
    }

    with open("articles.json", "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)

    print("  Done! Written " + str(len(enriched)) + " articles to articles.json")


if __name__ == "__main__":
    main()
